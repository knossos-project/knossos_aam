...
class Task(models.Model):
    ...
    category_name_combination = models.CharField(
        max_length=1000, unique=True, blank=True, null=False)
    ...
    # This is used to sort tasks for display to users, high integer here
    # means high priority.
    ...
    checks = models.CharField(
        max_length=400, blank=True, help_text=checks_available())
    ...
    task_file = models.FileField(
        default=False, upload_to=task_filename, null=True, blank=True)
    ...
    def oldest_submission(self):
        ...
        all_submissions_sorted = sorted(
            all_submissions, key=lambda x: x.date)
    ...
    def __unicode__(self):
        return "Task: " + " / ".join(
            [self.category.project.name, self.category.name, self.name])
...
pre_save.connect(submission_work_enforce_frozen, sender=Submission)
pre_save.connect(submission_ensure_valid_path, sender=Submission)
post_save.connect(work_update_post_submission, sender=Submission)

post_save.connect(user_username_without_dashes, sender=User)
post_save.connect(user_create_employee, sender=User)

pre_save.connect(submission_work_enforce_frozen, sender=Work)
post_save.connect(task_update_post_work_creation, sender=Work)
post_delete.connect(task_update_post_work_deletion, sender=Work)

post_save.connect(project_name_without_dashes, sender=Project)

post_save.connect(taskcategory_name_without_dashes, sender=TaskCategory)

pre_save.connect(task_name_without_dashes, sender=Task)
pre_save.connect(task_category_name_combination, sender=Task)
pre_save.connect(task_ensure_valid_path, sender=Task)
post_save.connect(task_validate_checks, sender=Task)
